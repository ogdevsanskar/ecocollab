import { API_CONFIG, type ForestChangeData, type ClimateData, type SatelliteImageData } from "../config/api-config.js"
...
export class GlobalForestWatchService {
  ...
  static async getDeforestationAlerts(lat: number, lng: number, radius = 10): Promise<ForestChangeData> {
    try {
      ...
      return data as ForestChangeData
    } catch (error) {
      console.error("[v0] Failed to fetch deforestation alerts, using fallback data:", (error as Error).message || error)
      return {
        alerts: [
...
export class NASAEarthDataService {
  ...
  static async getSatelliteImagery(lat: number, lng: number, date: string): Promise<SatelliteImageData> {
    try {
      ...
      return data as SatelliteImageData
    } catch (error) {
...
export class OpenWeatherService {
  ...
  static async getCurrentWeather(lat: number, lng: number) {
    try {
      ...
      console.error("[v0] Failed to fetch weather data, using fallback:", (error as Error).message || error)
      return {
  ...
  static async getAirPollution(lat: number, lng: number) {
    try {
      ...
      console.error("[v0] Failed to fetch air pollution data, using fallback:", (error as Error).message || error)
      return {
        list: [
          {
            main: { aqi: 2 },
...
export class MapboxService {
  ...
  static async reverseGeocode(lat: number, lng: number) {
    try {
      ...
      return (data as any).features[0]?.place_name || `${lat}, ${lng}`
    } catch (error) {
